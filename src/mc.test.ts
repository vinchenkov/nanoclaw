...
function runMc(baseDir: string, args: string[]): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [join(process.cwd(), "groups/shared/bin/mc.ts"), "--base-dir", baseDir, ...args], {
      cwd: process.cwd(),
...
describe("mc task create", () => {
  ...
  it("rejects task creation when the structured field is empty", async () => {
    ...
    ).rejects.toThrow(/Task requires at least one structured acceptance_criteria/i);
  });
});
